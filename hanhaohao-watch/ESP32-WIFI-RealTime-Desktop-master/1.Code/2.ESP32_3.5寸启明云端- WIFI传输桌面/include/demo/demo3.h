...
static void btn_event_cb(lv_obj_t * btn, lv_event_t event)
{
    if(event == LV_EVENT_CLICKED) {
        ...
        lv_label_set_text_fmt(label, "Button 3: %d", cnt);
    }
}
...
void lv_ex_get_started_2(void)
{
    ...
    lv_obj_set_pos(btn3, 10, 160);                            /*Set its position*/
    lv_obj_set_size(btn3, 120, 50);                          /*Set its size*/
    lv_obj_set_event_cb(btn3, btn_event_cb);                 /*Assign a callback to the button*/

    lv_obj_t * label2 = lv_label_create(btn3, NULL);          /*Add a label to the button*/
    lv_label_set_text(label2, "Button 3");                     /*Set the labels text*/
}
